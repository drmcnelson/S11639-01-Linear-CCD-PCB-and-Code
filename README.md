...
PCB, firmware and host software for the Hamamatsu S11639-01 Linear CCD with 16 bit ADC and SPI interface, intended for Teensy 4, Arduino, and Raspberry family devices.  The Hamamatsu S11639-01 is spec'd at 0.5mV dark noise, compared to 5mV for the Toshiba TC1304.  Response is similar to the Toshiba and Sony devices, but the dynamic range is larger at 8,000 (compared to approximately 300), largely due to the reduced noise and dark level.  The sensor makes good use of a low noise high precision front end and 16bit ADC.

This repository provides electrical design files for the Hamamatus S11639-01 linear CCD sensor board as described, and for a carrier for the Teensy 4.0 that interfaces to the sensor board with a 20 pin ribbon cable.  The sensor specific codes is in a separate c++ file and header, and there is a "sketch" that so far has run on a Teeensy 4.  The codes use the standard Arduino libraries plus a small number of optional register level enhancements for the i.MX RT MCU, that provide faster and more constant interrupt latency and SPI transfers with less overhead.
...
The external logic level is set by VDD.
